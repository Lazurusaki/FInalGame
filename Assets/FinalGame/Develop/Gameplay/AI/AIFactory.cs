...
namespace FinalGame.Develop.Gameplay.AI
{
    public class AIFactory
    {
        ...
        //public AIStateMachine CreateBomberBehavior(Entity entity)
        //    => CreateBomberStateMachine(entity);
        ...
        public AIStateMachine CreateBomberBehavior(Entity entity, ITargetSelector targetSelector)
        {
            List<IDisposable> disposables = new List<IDisposable>();
            ...
            Idle idleState = new Idle();
            Follow followState = new Follow(_container.Resolve<EntitiesBuffer>(), entity, moveDirection,
                rotationDirection, targetSelector);
            Detonate detonateState = new Detonate(entity);
            ...
            */
            ...
            //FuncCondition idleToFollowStateCondition = new FuncCondition(() =>
            //    targetSelector.TrySelectTarget(entity.GetDetectedEntitiesBuffer(), out Entity selectedTarget));

            FuncCondition followToDetonateStateCondition = new FuncCondition(() =>
                targetSelector.TryGetDistanceToTarget(out Entity target, out float distance) && distance < entity.GetDetonateDistance().Value); 
            
            AIStateMachine stateMachine = new AIStateMachine(disposables);
            
            stateMachine.AddState(followState);
            stateMachine.AddState(detonateState);
            
            stateMachine.AddTransition(followState, detonateState, followToDetonateStateCondition);
